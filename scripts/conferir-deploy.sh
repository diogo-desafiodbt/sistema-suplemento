...
# um serviço só.
echo "1/6 os serviços estabilizaram?"
...
echo "3/6 as páginas respondem? (uma por serviço)"
...
echo "4/6 os jobs voltaram? (espera até 7 min pelo poll de 5 em 5)"
...
echo "5/6 o vigia achou algo novo?"
...
echo "6/6 o código tem todas as variáveis que pede?"
# Existe por causa de 24/08/2026: o SUPPORT_IMAP_HOST foi criado no Secrets
# Manager e NUNCA ligado ao contêiner. O código exige quatro variáveis, três
# estavam ligadas, e ele desistia no começo — registrando sucesso. O poll de
# suporte rodou de 5 em 5 minutos por dias sem ler um único e-mail. A mesma
# coisa já tinha acontecido com HOTMART2_*.
#
# Criar o segredo não é ligar o segredo. Esta pergunta é a única aqui que olha
# a CONFIGURAÇÃO em vez do comportamento, porque este defeito não produz
# sintoma nenhum: nada quebra, nada alerta, o trabalho só não acontece.
#
# Ausência aceita, com o motivo:
#   AWS_REGION, AWS_DEFAULT_REGION  o próprio ECS injeta em tempo de execução
#   ANTHROPIC_SUPPORT_MODEL         tem padrão no código (src/lib/support/ai.ts)
ACEITAS="AWS_REGION AWS_DEFAULT_REGION ANTHROPIC_SUPPORT_MODEL"
PEDIDAS=$(grep -rhoE 'process\.env\.[A-Z][A-Z0-9_]*' src/ | sed 's/process\.env\.//' | sort -u)
LIGADAS=$(for S in $SERVICOS; do
  TD=$(aws ecs describe-services --region "$REGIAO" --cluster "$CLUSTER" --services "$S" \
       --query 'services[0].taskDefinition' --output text)
  aws ecs describe-task-definition --region "$REGIAO" --task-definition "$TD" \
    --query 'taskDefinition.containerDefinitions[0].[secrets[].name,environment[].name]' \
    --output text | tr '\t' '\n'
done | sort -u)
FALTANDO=""
for V in $PEDIDAS; do
  echo "$LIGADAS" | grep -qx "$V" && continue
  echo "$ACEITAS" | tr ' ' '\n' | grep -qx "$V" && continue
  FALTANDO="$FALTANDO $V"
done
[ -z "$FALTANDO" ] && ok "toda variável pedida pelo código está ligada" \
  || falha "o código pede e ninguém liga:$FALTANDO" \
     "ver db/aws/sistema-suplemento-taskdef.json — registrar revisão nova a partir dele"
