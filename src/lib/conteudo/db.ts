...
const globalForDb = globalThis as unknown as { __ddSqlConteudo?: Sql }
...
export type ConteudoTable = keyof typeof CONFLICT_COLUMNS
...
/** Conexão ao banco `conteudo` (usuário `job_conteudo`, token IAM). */
export function getSqlConteudo(): Sql {
  if (!globalForDb.__ddSqlConteudo) {
    globalForDb.__ddSqlConteudo = createSql()
  }
  return globalForDb.__ddSqlConteudo
}

/** Upsert em lote; retorna linhas afetadas (`result.count`). Lote vazio → 0, sem query. */
export async function upsertConteudo(
  sql: Sql,
  table: ConteudoTable,
  ...
  const updateKeys = Object.keys(rows[0]).filter(
    (k) => !(conflictCols as readonly string[]).includes(k),
  )
  const onConflict =
    updateKeys.length > 0
