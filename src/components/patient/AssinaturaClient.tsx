...
import { canCancelRecurringBilling, PLAN_LABELS } from '@/lib/plans'
...
export function AssinaturaClient({ subscription, payments }: Props) {
  ...
  async function handleCancel() {
    ...
    try {
      ...
      toast.error(
        'Erro ao cancelar. Tente novamente ou entre em contato com o suporte.',
      )
    } finally {
      setLoading(false)
    }
      ...
      <div className="space-y-4">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <span
              className={`text-xs font-bold px-3 py-1 rounded-full ${statusInfo.className}`}
            >
              {statusInfo.label}
            </span>
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 space-y-4">
            <h2 className="text-lg font-bold text-[#13244f]">
              Cancelar renovação?
            </h2>
            <p className="text-sm text-gray-500 leading-relaxed">
              Ao cancelar, sua assinatura não será renovada. Você continuará com
              acesso ao tratamento até{' '}
              <strong className="text-[#13244f]">{expiresAt}</strong> e, após
              essa data, o acesso será encerrado automaticamente.
            </p>
            <p className="text-sm text-gray-500">
              Para retomar o tratamento depois, será necessário iniciar um novo
              protocolo.
            </p>
