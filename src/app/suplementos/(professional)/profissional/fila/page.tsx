      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-bold px-3 py-1.5 rounded-full bg-amber-100 text-amber-700">
            {pendingProtocols.length} pendente
            {pendingProtocols.length !== 1 ? 's' : ''}
          </span>
        </div>
        ...
        {pendingProtocols.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 text-center">
            ...
            <p className="text-[#13244f] font-semibold">
              Nenhuma prescrição pendente
            </p>
            <p className="text-gray-400 text-sm mt-1">
              Todas as prescrições foram assinadas.
            </p>
          </div>
        ) : (
          <div className="space-y-3">
            {pendingProtocols.map((protocol) => {
              ...
              const generatedAt = new Date(
                protocol.generated_at,
              ).toLocaleDateString('pt-BR')

              return (
                <Link
                  key={protocol.id}
                  href={`/suplementos/profissional/protocolo/${protocol.id}`}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-2 flex-1">
                      <div className="flex items-center gap-3 flex-wrap">
                        <span className="font-bold text-[#13244f]">
                          {protocol.users?.full_name}
                        </span>
                        <span className="font-mono text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded">
                          {protocol.users?.client_code}
                        </span>
                        <span
                      ...
                      <p className="text-sm text-gray-400">
                        {protocol.users?.email}
                      </p>
                      <div className="flex gap-2 flex-wrap">
                        {activeItems?.map((item) => (
                          <span
                            key={item.products?.name}
                            className="text-xs bg-[#13244f]/5 text-[#13244f] font-medium px-2.5 py-1 rounded-full border border-[#13244f]/10"
                          >
                            {item.products?.name}
                          </span>
                        ))}
                    ...
                    <div className="text-right flex-shrink-0 space-y-1">
                      ...
                      <p className="text-xs text-[#f4001e] font-semibold">
                        Assinar →
                      </p>
                    </div>
                  </div>
