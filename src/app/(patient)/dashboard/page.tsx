      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Código do cliente: {profile?.client_code}
          </p>
        </div>
        ...
        {!hasTreatment ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center space-y-4">
            ...
            <p className="text-[#13244f] font-medium">
              Você ainda não tem um tratamento ativo.
            </p>
            <Link
              href="/quiz"
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`text-xs font-bold px-3 py-1 rounded-full ${getPatientOrderStatusColor(orderStatus)}`}
                >
                  {orderStatus}
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                {activeItems?.map((item) => (
                  <span
                    ...
                    className="text-xs bg-[#13244f]/5 text-[#13244f] font-medium px-3 py-1 rounded-full border border-[#13244f]/10"
                  >
                    {item.products?.name}
                  </span>
                ))}
              ...
              <Link
                ...
                className="inline-block mt-4 text-sm text-[#f4001e] font-semibold hover:underline"
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              <Link
                ...
                className={`bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-2 transition ${hasDiet ? 'hover:border-[#13244f]/30' : 'opacity-50 pointer-events-none'}`}
              >
                <span className="text-lg">🥗</span>
                <p className="text-sm font-bold text-[#13244f]">Minha Dieta</p>
                <p className="text-xs text-gray-400">
                  {hasDiet ? 'Disponível' : 'Plano anual'}
                </p>
              </Link>
              <Link
                href="/dashboard/guia"
                className={`bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-2 transition ${hasGuide ? 'hover:border-[#13244f]/30' : 'opacity-50 pointer-events-none'}`}
              >
                <span className="text-lg">📖</span>
                <p className="text-sm font-bold text-[#13244f]">Guia Digital</p>
                <p className="text-xs text-gray-400">
                  {hasGuide ? 'Disponível' : 'Compra necessária'}
                </p>
              </Link>
            </div>
          </div>
