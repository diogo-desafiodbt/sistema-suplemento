  ...
  return (
    <div className="min-h-screen bg-[#f5f0eb]">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-5">
        <div>
          <Link href="/dashboard/pedidos" className="text-xs text-[#13244f]/60 hover:text-[#13244f] transition">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold tracking-widest text-[#13244f]/50 uppercase mb-1">
              Pedido de {new Date(orderData.created_at).toLocaleDateString('pt-BR')}
            </p>
            <h1 className="text-2xl font-bold text-[#13244f]">Detalhes do pedido</h1>
          </div>
          <span className={`text-xs font-bold px-3 py-1 rounded-full ${getPatientOrderStatusColor(statusMessage)}`}>
            {statusMessage}
          </span>
        </div>
        ...
        {delivered ? (
          deliveredEvent?.datahora && (
            <div className="bg-green-50 border border-green-100 rounded-2xl px-5 py-4 text-sm text-green-800 font-semibold">
              Entregue em {new Date(deliveredEvent.datahora).toLocaleDateString('pt-BR')}
            </div>
          )
        ) : (
          estimatedDate && (
            <div className="bg-[#13244f]/5 border border-[#13244f]/10 rounded-2xl px-5 py-4 text-sm text-[#13244f] font-semibold">
              Previsão de entrega: até {estimatedDate.toLocaleDateString('pt-BR')}
            </div>
          )
        )}
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-xs font-bold tracking-widest text-[#13244f]/50 uppercase mb-4">Produtos</p>
              ...
              return (
                <div key={`${name}-${i}`} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-[#13244f]">{name}</p>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-xs font-bold tracking-widest text-[#13244f]/50 uppercase mb-3">Rastreio</p>
          {orderData.tracking_code ? (
            <div className="flex flex-wrap items-center gap-3">
              <span className="font-mono text-sm text-[#13244f] bg-[#f5f0eb] px-3 py-1.5 rounded-lg">
                {orderData.tracking_code}
              </span>
              <CopyButton value={orderData.tracking_code} label="Copiar código" />
            </div>
          ) : (
            <p className="text-sm text-gray-400">
              O código de rastreio aparece aqui assim que o pedido for despachado.
            </p>
          )}
          ...
          <div className="mt-5 border-t border-gray-100 pt-4">
            {eventos.length === 0 ? (
              <p className="text-sm text-gray-400">Aguardando atualização de rastreio</p>
            ) : (
              <ol>
                {eventos.map((ev, i) => (
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-xs font-bold tracking-widest text-[#13244f]/50 uppercase mb-3">Endereço de entrega</p>
          {hasAddress ? (
            <div className="text-sm text-[#13244f]">
              <p>
                {pharmacy!.EntregaLogradouro}, {pharmacy!.EntregaLogradouroNumero}
              ...
              <p className="text-gray-500 mt-0.5">
                ...
                {pharmacy!.EntregaCEP ? ` — CEP ${fmtCep(pharmacy!.EntregaCEP)}` : ''}
              </p>
            </div>
          ) : (
            <p className="text-sm text-gray-400">
              O endereço de entrega será confirmado quando o pedido for processado.
            </p>
          )}
        </section>
        ...
        {/* 2.5 — Pagamento */}
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-xs font-bold tracking-widest text-[#13244f]/50 uppercase mb-3">Pagamento</p>
