...
### `buscar_conteudo` ainda não tem base

Ele deveria buscar por semelhança nas transcrições do canal. **Essas tabelas
não existem no RDS** — ficaram na Supabase, que está pausada. Estou trazendo.

Então: **escreva a ferramenta agora, devolvendo sempre vazio**, com um
comentário dizendo por quê e o que falta. Quando a base chegar, liga-se a
consulta e nada mais muda.

Quando existir: devolve **apenas `{ titulo, url }`**, nunca o texto do trecho.
Se a IA lesse o conteúdo, parafrasearia — e paráfrase de conteúdo médico é
conselho médico. Sem semelhança suficiente, devolve vazio: link mal escolhido
...
6. Todo envio por `mailer.ts` sai com o rodapé, sem quem chama precisar pedir.
7. A migration existe em `db/clinico/` e **não foi executada**.
